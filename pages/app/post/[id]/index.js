...
export default function Post() {
  ...
  const publish = async () => {
    ...
    const devUrl = `http://${post.site.subdomain}.localhost:3000/${post.slug}`;
    const prodUrl = `https://${post.site.subdomain}.myhomecomp.com/${post.slug}`;
    const prod = process.env.NODE_ENV === 'production';

    if (response.ok) {
      mutate(`/api/post?postId=${postId}`);
      router.push(prod ? prodUrl : devUrl);
    }
  };
